...
const inputCls = 'w-full px-3 py-2 text-sm border border-gray-200 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 placeholder:text-gray-400 dark:placeholder:text-gray-500 focus:outline-none focus:ring-2 focus:ring-blue-500';

const RegisterModal = memo(function RegisterModal({ onu, olt, oltId, vlans, onClose, onSuccess }: RegisterModalProps) {
  ...
  const hasMikrotik = olt.has_mikrotik;

  const handleToggleCustomer = (v: boolean) => {
    setCreateCustomer(v);
    if (!v) setShowPppoe(false);
  };
    ...
    try {
      ...
      if (createCustomer) {
        try {
          ...
          if (hasMikrotik && pppoeUser.trim()) {
            payload.pppoe_username = pppoeUser.trim();
            payload.pppoe_password = pppoePass.trim();
          }
      ...
      <div className="bg-white dark:bg-gray-900 rounded-2xl shadow-2xl w-full max-w-md flex flex-col max-h-[90vh]">
        ...
        <div className="px-6 pt-6 pb-4 border-b border-gray-100 dark:border-gray-800 shrink-0">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-xl bg-linear-to-br from-blue-50 to-indigo-100 dark:from-blue-900/30 dark:to-indigo-900/30 flex items-center justify-center text-blue-600 shrink-0">
              <Play className="h-5 w-5" />
            </div>
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2 flex-wrap">
                <h3 className="text-base font-bold text-gray-900 dark:text-white">Register ONU</h3>
                {hasMikrotik && (
                  <span className="inline-flex items-center gap-1 px-1.5 py-0.5 rounded-md bg-cyan-50 dark:bg-cyan-900/30 text-cyan-700 dark:text-cyan-300 text-[10px] font-semibold uppercase tracking-wide border border-cyan-200 dark:border-cyan-700">
                    MikroTik
                  </span>
                )}
              </div>
              <p className="text-xs text-gray-400 dark:text-gray-500 font-mono mt-0.5">{onu.serial_number} · {onu.pon_port}</p>
            </div>
          </div>
        </div>
        ...
        <div className="overflow-y-auto flex-1 px-6 py-4 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="text-xs font-medium text-gray-600 dark:text-gray-400 block mb-1.5">VLAN</label>
              <select className={inputCls} value={vlanId} onChange={e => setVlanId(e.target.value)}>
                <option value="">No VLAN</option>
                {vlans.map(v => (
                  <option key={v.id} value={v.vlan_id}>{v.name} ({v.vlan_id})</option>
                ))}
              </select>
            </div>
            <div>
              <label className="text-xs font-medium text-gray-600 dark:text-gray-400 block mb-1.5">Description</label>
              <input
                className={inputCls}
                value={description}
          ...
          <div className="rounded-xl border border-gray-200 dark:border-gray-700 overflow-hidden">
            <button
              type="button"
              onClick={() => handleToggleCustomer(!createCustomer)}
              className="w-full flex items-center justify-between px-4 py-3 hover:bg-gray-50 dark:hover:bg-gray-800/50 transition-colors"
            >
              <div className="flex items-center gap-2.5">
                <Users className="h-4 w-4 text-blue-500" />
                <span className="text-sm font-medium text-gray-800 dark:text-gray-200">Link Customer</span>
              </div>
              <div className={clsx(
              ...
              <div className="border-t border-gray-100 dark:border-gray-800 bg-gray-50/60 dark:bg-gray-800/30">
                <div className="px-4 pt-3 pb-4 grid grid-cols-2 gap-3">
                  <div className="col-span-2">
                    <label className="text-xs font-medium text-gray-600 dark:text-gray-400 block mb-1.5">Name <span className="text-red-500">*</span></label>
                    <input value={custName} onChange={e => setCustName(e.target.value)} placeholder="Full name" className={inputCls} />
                  </div>
                  <div>
                    <label className="text-xs font-medium text-gray-600 dark:text-gray-400 block mb-1.5">Phone</label>
                    <input value={custPhone} onChange={e => setCustPhone(e.target.value)} placeholder="03XX-XXXXXXX" className={inputCls} />
                  </div>
                  <div>
                    <label className="text-xs font-medium text-gray-600 dark:text-gray-400 block mb-1.5">Plan</label>
                    <input value={custPlan} onChange={e => setCustPlan(e.target.value)} placeholder="e.g. 10 Mbps" className={inputCls} />
                  </div>

                  {/* PPPoE — only if OLT has MikroTik */}
                  {hasMikrotik && (
                    <>
                      <div className="col-span-2 border-t border-gray-200 dark:border-gray-700 pt-3">
                        ...
                        >
                          <div className="flex items-center gap-2">
                            ...
                            <span className="text-xs font-semibold text-cyan-700 dark:text-cyan-300 uppercase tracking-wide">PPPoE / MikroTik</span>
                          </div>
                          <div className={clsx(
                            'relative w-9 h-5 rounded-full transition-colors shrink-0',
                            showPppoe ? 'bg-cyan-500' : 'bg-gray-200 dark:bg-gray-600'
                          )}>
                            <span className={clsx(
                              'absolute top-0.5 left-0.5 w-4 h-4 rounded-full bg-white shadow transition-transform',
                              showPppoe && 'translate-x-4'
                            )} />
                          </div>
                        </button>
                      </div>
                      {showPppoe && (
                        <>
                          <div>
                            <label className="text-xs font-medium text-gray-600 dark:text-gray-400 block mb-1.5">PPPoE Username</label>
                            <input value={pppoeUser} onChange={e => setPppoeUser(e.target.value)} placeholder="username" className={inputCls} />
                          </div>
                          <div>
                            <label className="text-xs font-medium text-gray-600 dark:text-gray-400 block mb-1.5">PPPoE Password</label>
                            <input type="password" value={pppoePass} onChange={e => setPppoePass(e.target.value)} placeholder="password" className={inputCls} />
                          </div>
                        </>
                      )}
                    </>
          ...
          <div className="flex items-start gap-2 bg-blue-50 dark:bg-blue-900/20 border border-blue-100 dark:border-blue-800/50 rounded-lg px-3 py-2.5 text-xs text-blue-700 dark:text-blue-300">
            <CheckCircle className="h-3.5 w-3.5 shrink-0 mt-0.5" />
            Auto-provisioned via SNMP / Telnet / Hybrid — no CLI needed.
          </div>
        </div>
        ...
        <div className="flex gap-3 px-6 py-4 border-t border-gray-100 dark:border-gray-800 shrink-0">
          ...
          <Button className="flex-1" loading={loading} onClick={handleRegister} icon={<Play className="h-4 w-4" />}>
            Register ONU
          </Button>
        </div>
      </div>
...
export default function ONUManagementPage() {
  ...
  if (isLoading) return <ONUPageSkeleton />;
