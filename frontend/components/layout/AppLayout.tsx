          ...
          {isAdmin && (
            <div ref={adminRef} className="relative">
              <button
                ...
                type="button"
              >
                <Settings className={clsx('h-3.5 w-3.5 transition-transform duration-300', adminOpen && 'rotate-45')} />
                <span className="hidden sm:inline">Admin</span>
                <ChevronRight className={clsx('h-3 w-3 transition-transform hidden sm:block', adminOpen && 'rotate-90')} />
              </button>

              {adminOpen && (
                <div className="absolute right-0 top-full mt-2 w-56 bg-white dark:bg-gray-800 rounded-xl shadow-xl shadow-gray-200/60 dark:shadow-gray-950/60 border border-gray-100 dark:border-gray-700/80 z-50 overflow-hidden">
                  {/* Header */}
                  <div className="px-3 py-2 bg-linear-to-r from-violet-600 to-purple-600 flex items-center gap-2">
                    <Settings className="h-3.5 w-3.5 text-violet-200 shrink-0" />
                    <span className="text-xs font-semibold text-white tracking-wide">Administration</span>
                  </div>
                  <div className="p-1.5 space-y-0.5">
                    {[
                      ...
                      return (
                        ...
                        >
                          <Icon className={clsx('h-4 w-4 shrink-0', active ? 'text-violet-500' : 'text-gray-400 dark:text-gray-500')} />
                          <div className="min-w-0 flex-1">
                            <p className="text-sm font-medium leading-none">{label}</p>
                            <p className="text-[11px] text-gray-400 dark:text-gray-500 mt-0.5">{desc}</p>
                          </div>
                          {active && <span className="w-1.5 h-1.5 rounded-full bg-violet-500 shrink-0" />}
                        </Link>
                      );
                    })}
                  </div>
